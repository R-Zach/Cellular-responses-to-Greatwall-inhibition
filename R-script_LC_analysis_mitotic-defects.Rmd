...
ggsave(paste(path_results, "graph_defective_mitosis.pdf", sep = ""), graph_mitotic_defects,
       width = 72.5, height = 20, units = "mm", device = cairo_pdf)
```
...
graph_mitotic_timing <- ggplot() +
  ...
  geom_text(data = data_graph_p, aes(x = C604, y = 1750, label = label), size = 2.11, family = "Arial") +
  
  facet_grid(. ~ cell_line) +
  labs(x = "C-604 (µM)", y = "Mitotic timing\n(min)") +
...
## Plotting a correlation scatterplot of frequencies of mitotic exit defects and mitotic timing
```{r}
data_graph <- data_raw %>%
  mutate(cell_line = factor(cell_line, levels = c("HCC1395", "U2OS", "MM231", "RPE-1", "HeLa", "BT-549")),
         treatment = factor(C604, levels = c(0, 0.25, 0.5, 1, 2)))

data_graph_summary <- data_graph %>%
  reframe(timing_median = median(timing), .by = c(experiment, cell_line, treatment, outcome))

data_stats <- data_graph_summary %>%
  group_by(cell_line, treatment) %>%
  group_split() %>%
  lapply((function (data) {
    
    tmp.normal <- filter(data, outcome == "normal")$timing_median
    tmp.defect <- filter(data, outcome == "defect")$timing_median
    
    if (length(tmp.normal) == length(tmp.defect)) {
      
      tmp.stats <- t.test(tmp.normal, tmp.defect,
                          alternative = "two.sided",
                          paired = FALSE,
                          var.equal = TRUE)
      
      tmp.outcome <- data.frame(cell_line = unique(data$cell_line),
...
graph_timing_defects <- ggplot(data = data_graph) +
  
  geom_point(aes(x = treatment, y = timing, group = outcome, fill = outcome),
             size = 1, shape = 21, alpha = 0.25, stroke = NA,
             position = position_jitterdodge(0.25)) +
  
  geom_point(data = data_graph_summary, aes(x = treatment, y = timing_median, group = outcome, colour = outcome),
             fill = "#FFFFFF",
             size = 0.5, stroke = 0.3, shape = 21,
             position = position_jitterdodge(0.25)) +
  
  geom_text(data = filter(data_stats, significance %in% c("ns")), aes(x = treatment, y = 2500, label = significance),
            family = "Arial", size = 5/.pt, colour = colour_pallette[2]) +
  
  geom_text(data = filter(data_stats, significance %in% c("nd")), aes(x = treatment, y = 2500, label = significance),
            family = "Arial", size = 5/.pt, colour = colour_pallette[1]) +
  
  geom_text(data = filter(data_stats, !(significance %in% c("nd", "ns"))), aes(x = treatment, y = 1850, label = significance),
            family = "Arial", size = 6/.pt, colour = colour_pallette[6]) +
  
  scale_fill_manual(values = c(colour_pallette[3], colour_pallette[1])) +  
  scale_colour_manual(values = c(colour_pallette[5], colour_pallette[6])) +
  scale_y_continuous(expand = c(0, 0), trans = "log2", breaks = c(20, 80, 320, 1280), limits = c(18, 4000)) +
  labs(x = "GWL inhibitor C-604 (µM)", y = "Timing (min)") +
  
  facet_grid(. ~ cell_line) +
  graph_theme +
  theme(axis.text.x = element_text(colour = "#000000", size = 6, margin = margin(t = 2), angle = -90, hjust = 0, vjust = 0.5, family = "Arial"))
  
ggsave(paste(path_results, "graph_timing_defects.pdf", sep = ""), graph_timing_defects,
       width = 80, height = 22.5, units = "mm", device = cairo_pdf)
```

## Creating a composite graph
```{r}
graph_composite <- plot_grid(graph_mitotic_defects,
                             graph_mitotic_timing,
                             graph_corr_distance_defects,
                             nrow = 3, ncol = 1, align = "h")

ggsave(paste(path_results, "graph_composite.pdf", sep = ""), graph_composite,
       width = 72.5, height = 75, units = "mm", device = cairo_pdf)
```
