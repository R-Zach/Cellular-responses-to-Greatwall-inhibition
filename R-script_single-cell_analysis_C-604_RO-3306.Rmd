---
title: "R-script_IF_single-cell_analysis_C-604_RO-3306"
...
```{r}
path_data <- "/Users/rz73/Library/CloudStorage/OneDrive-UniversityofSussex/Submissions/Nature Communications (2025) - Cellular consequences of Greatwall inhibition/Supplementary code (testing)/R-script_IF_single-cell_analysis_C-604_RO-3306/data/IF_single-cell_C-604_RO-3306.csv"
path_results <- "/Users/rz73/Library/CloudStorage/OneDrive-UniversityofSussex/Submissions/Nature Communications (2025) - Cellular consequences of Greatwall inhibition/Supplementary code (testing)/R-script_IF_single-cell_analysis_C-604_RO-3306/results/"
```
...
## Normalising data and asigning cell cycle groups
```{r}
...
data_stats_cell_cycle <- data_cell_cycle_proportions %>%
  filter(RO3306 == 0, C604 %in% c(0, 0.25, 0.5, 1, 2), cell_cycle %in% c("4N", "8N+")) %>%
  ...
  lapply((function (data) {
    ...
    statistics <- fun_stats(data, group_vars = c("cell_line", "C604"), val = "prop") %>%
...
data_stats_p21 <- data_p21_proportions %>%
  filter(RO3306 == 0, C604 %in% c(0, 0.25, 0.5, 1, 2), p21 == "positive") %>%
  complete(experiment, cell_line, C604, RO3306, p21) %>%
  replace(is.na(.), 0) %>%
  fun_stats(group_vars = c("cell_line", "C604"), val = "prop") %>%
  filter(group_1 != group_2)
...
## Computing statistics for the proportions of multinucleated cells
```{r}
data_stats_multinucleated <- data_multinucleated_proportions  %>%
  filter(RO3306 == 0, C604 %in% c(0, 0.25, 0.5, 1, 2), nuclei %in% c("2", "3+")) %>%
  ...
  lapply((function (data) {
    ...
    tmp.nuclei <- unique(data$nuclei)
    statistics <- fun_stats(data, group_vars = c("cell_line", "C604"), val = "prop") %>%
      mutate(nuclei = tmp.nuclei)
    return(statistics)
...
data_stats_multinucleated_subset <- data_stats_multinucleated %>%
  separate(group_1, into = c("cell_line_1", "C604_1"), sep = "_", remove = FALSE) %>%
  separate(group_2, into = c("cell_line_2", "C604_2"), sep = "_", remove = FALSE) %>%
  filter(cell_line_1 == cell_line_2, C604_1 == 0) %>%
  mutate(shapiro_1_adj = p.adjust(shapiro_1, method = "BH"),
...
data_stats_multinucleated_subset
```
## Establishing cell_area summary analysis
```{r}
data_cell_area_summary <- data_normalised %>%
...
## Computing statistics for median cell areas
```{r}
data_stats_cell_area <- data_cell_area_summary %>%
  complete(experiment, cell_line, C604) %>%
  replace(is.na(.), 0) %>%
  fun_stats(group_vars = c("cell_line", "C604"), val = "area_cell_norm_median") %>%
  filter(group_1 != group_2)

data_stats_cell_area_subset <- data_stats_cell_area %>%
  separate(group_1, into = c("cell_line_1", "C604_1"), sep = "_", remove = FALSE) %>%
  separate(group_2, into = c("cell_line_2", "C604_2"), sep = "_", remove = FALSE) %>%
  filter(cell_line_1 == cell_line_2, C604_1 == 0) %>%
  mutate(shapiro_1_adj = p.adjust(shapiro_1, method = "BH"),
...
## Establishing maxima of synergy values
```{r}
data_synergy_max_summary <- data_synergy %>%
  reframe(S_HSA_mean = mean(S_HSA), S_HSA_sd = sd(S_HSA),
          .by = c(cell_line, RO3306, C604)) %>%
  group_by(cell_line) %>%
  filter(S_HSA_mean == max(S_HSA_mean))

data_synergy_max_summary
```
...
data_graph_points <- data_synergy_max_summary %>%
  mutate(cell_line = factor(cell_line, levels = list_cell_lines),
...
data_graph_stats_p21 <- data_graph_summary %>%
  filter(treatment %in% c(0, 0.125, 0.25, 0.5, 1, 2)) %>%
  complete(experiment, cell_line, treatment, cell_cycle) %>%
  replace(is.na(.), 0) %>%
  group_by(cell_cycle) %>%
  group_split() %>%
  lapply((function (data) {
    ...
    tmp.cell.cycle <- unique(data$cell_cycle)
    statistics <- fun_stats(data, group_vars = c("cell_line", "treatment"), val = "p21_median") %>%
      mutate(cell_cycle = tmp.cell.cycle)
    return(statistics)
...
graph_p21_points <- ggplot() +
  ...
  scale_alpha_continuous(range = c(0.1, 1)) +
  facet_nested(dataset + factor(treatment) ~ cell_line, strip = strip_nested(clip = "off")) +
  graph_theme +
  theme(
    axis.text.y = element_text(colour = "#000000", size = 6, margin = margin(r = 0), family = "Arial"),
    axis.title.x = element_text(colour = "#000000", size = 6, margin = margin(t = 2), family = "Arial"),
...
data_stats <- data_stats_cell_cycle_subset %>%
  dplyr::rename(p = ttest_p_adj, cell_line = cell_line_2, treatment = C604_2) %>%
...
data_stats <- data_stats_multinucleated_subset %>%
  dplyr::rename(p = ttest_p_adj, cell_line = cell_line_2, treatment = C604_2) %>%
  mutate(significance = case_when(p < 0.001 ~ "***",
                                  p < 0.01 ~ "**",
                                  p < 0.05 ~ "*",
...
graph_multinucleated_proportions_C604_only <- ggplot() +
  ...
  geom_text(data = filter(data_stats, significance != "ns", nuclei == "2"),
            aes(x = x_axis, y = 112, label = significance), size = 6/.pt, colour = colour_pallette[3], family = "Arial") +
  geom_text(data = filter(data_stats, significance != "ns", nuclei == "3+"),
            aes(x = x_axis, y = 102, label = significance), size = 6/.pt, colour = colour_pallette[5], family = "Arial") +
  geom_text(data = filter(data_stats, significance == "ns"), 
            aes(x = x_axis, y = 115, label = significance), size = 5.5/.pt, colour = colour_pallette[1], family = "Arial") +
...
data_graph_cell_area_summary <- data_cell_area_summary %>%
  dplyr::rename(treatment = C604) %>%
  mutate(cell_line = factor(cell_line, levels = list_cell_lines),
         treatment = factor(treatment, levels = c(0, 0.25, 0.5, 1, 2)))

data_stats <- data_stats_cell_area_subset %>%
  dplyr::rename(p = ttest_p_adj, cell_line = cell_line_2, treatment = C604_2) %>%
  mutate(significance = case_when(p < 0.001 ~ "***",
                                  p < 0.01 ~ "**",
                                  p < 0.05 ~ "*",
...
graph_cell_area_C604_only <- ggplot() +
  ...
  geom_hline(yintercept = c(0.21, 75), colour = colour_pallette[6], linetype = "solid", linewidth = 0.25) +
  
  geom_segment(data = data_graph, aes(x = 0.63, xend = 0.63, y = 0.21, yend = 75),
               colour = colour_pallette[6], linetype = "solid", linewidth = 0.25) +
  
  geom_segment(data = data_graph, aes(x = 5.37, xend = 5.37, y = 0.21, yend = 75),
               colour = colour_pallette[6], linetype = "solid", linewidth = 0.25) +
  
  geom_text(data = filter(data_stats, significance != "ns"),
            aes(x = treatment, y = 90, label = significance), size = 6/.pt, colour = colour_pallette[10], family = "Arial") +
  
  geom_text(data = filter(data_stats, significance == "ns"), 
            aes(x = treatment, y = 140, label = significance), size = 5.5/.pt, colour = colour_pallette[1], family = "Arial") +
...
## Plotting Shapiro-Wilk test results
```{r}
cell_line_list <- c("HCC1395", "U2OS", "RPE-1", "MM231", "HeLa", "BT-549")

data_graph <- bind_rows(
  
  dplyr::select(data_stats_cell_cycle_subset, group_2, shapiro_1_adj, shapiro_2_adj, cohen_d, dataset = cell_cycle) %>%
    mutate(dataset = case_when(dataset == "4N" ~ "Cell cycle (4N)",
                               dataset == "8N+" ~ "Cell cycle (8N+)",
                               .default = dataset)),
  
  dplyr::select(data_stats_p21_subset, group_2, shapiro_1_adj, shapiro_2_adj, cohen_d) %>%
    mutate(dataset = "p21+ cells"),
  
  dplyr::select(data_stats_multinucleated_subset, group_2, shapiro_1_adj, shapiro_2_adj, cohen_d, dataset = nuclei) %>%
    mutate(dataset = case_when(dataset == "2" ~ "Nuclei (2 nuclei)",
                               dataset == "3+" ~ "Nuclei (3+ nuclei)",
                               .default = dataset)),
  
  dplyr::select(data_stats_cell_area_subset, group_2, shapiro_1_adj, shapiro_2_adj, cohen_d) %>%
    mutate(dataset = "Cell area")) %>%
  
  separate(group_2, into = c("cell_line", "treatment"), sep = "_") %>%
  mutate(dataset = factor(dataset,
                          levels = c("Cell cycle (4N)", "Cell cycle (8N+)", "p21+ cells", "Nuclei (2 nuclei)", "Nuclei (3+ nuclei)", "Cell area"),
                          labels = c("Cell cycle\n(4N)", "Cell cycle\n(8N+)", "p21+ cells", "Nuclei\n(2 nuclei)", "Nuclei\n(3+ nuclei)", "Cell area")),
         cell_line = factor(cell_line, levels = cell_line_list))

data_graph_n <- data_graph %>%
  reframe(n = n(), .by = dataset)

graph_Shapiro_Wilk <- ggplot(data = data_graph, aes(x = shapiro_1_adj, y = shapiro_2_adj)) +
  
  geom_hline(yintercept = 0.05, linewidth = 0.25, colour = colour_pallette[2]) +
  geom_vline(xintercept = 0.05, linewidth = 0.25, colour = colour_pallette[2]) +
  labs(x = "Shapiro-Wilk test (p-value, group 1)", y = "Shapiro-Wilk test\n(p-value, group 2)") +
  
  geom_point(colour = colour_pallette[5], size = 1, alpha = 0.25, shape = 16) +
  facet_grid(. ~ dataset) +
  scale_x_continuous(trans = "log10", limits = c(10^-13, 1), breaks = c(10^-12, 10^-8, 10^-4, 1)) +
  scale_y_continuous(trans = "log10", limits = c(10^-13, 1), breaks = c(10^-12, 10^-10, 10^-8, 10^-6, 10^-4, 10^-2, 1)) +
  graph_theme +
...
graph_cohen_d <- ggplot(data = data_graph, aes(x = treatment, y = cell_line, size = abs(cohen_d), fill = dataset)) +
  
  geom_point(shape = 21, stroke = 0.25, colour = colour_pallette[6]) +
  scale_size(range = c(0, 2.5), breaks = c(0, 1, 2, 4, 8, 16, 32)) +
  labs(x = "Greatwall inhibitor C-604 (µM)", size = "Cohen's d (absolute)") +
  facet_grid(. ~ dataset) +
  scale_x_discrete(expand = c(0.125, 0.125)) +
  scale_y_discrete(expand = c(0.125, 0.125)) +
  scale_fill_manual(values = c(colour_pallette[4], colour_pallette[5], colour_pallette[7], colour_pallette[3], colour_pallette[5], colour_pallette[9])) +
  graph_theme +
  theme(panel.spacing.y = unit(0.25, "lines"),
        panel.spacing.x = unit(0.25, "lines"),
        axis.title.y = element_blank(),
...
ggsave(paste(path_results, "graph_Shapiro_Wilk.pdf", sep = ""), graph_Shapiro_Wilk,
       width = 80, height = 35, units = "mm", device = cairo_pdf)

ggsave(paste(path_results, "graph_cohen_d.pdf", sep = ""), graph_cohen_d,
       width = 80, height = 35, units = "mm", device = cairo_pdf)

graph_stats_composite <- plot_grid(
...
ggsave(paste(path_results, "graph_stats_composite_C604_only.pdf", sep = ""), graph_stats_composite,
       width = 160, height = 37.5, units = "mm", device = cairo_pdf)
```
